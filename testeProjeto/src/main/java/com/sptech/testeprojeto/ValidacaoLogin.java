...
public class ValidacaoLogin {
        ...
        public void run() {
                ...
                if (usoCPU > 30.0 && usoCPU <= 50.0) {
                        // List maquina = template.queryForList(
                        // "SELECT TOP (1) hostname FROM [dbo].[log_registros] JOIN [dbo].[maquinas] ON
                        // id_maquina = fk_maquina JOIN [dbo].[componentes] ON id_componente =
                        // fk_componente JOIN [dbo].[operacoes] ON id_operacao = fk_operacao WHERE tipo
                        // = 'Processador' AND uso > 30.0 AND uso <= 50.0 ORDER BY id_registro DESC");
                        // List localidade = template.queryForList(
                        // "SELECT TOP (1) localidade_maquina FROM [dbo].[log_registros] JOIN
                        // [dbo].[maquinas] ON id_maquina = fk_maquina JOIN [dbo].[componentes] ON
                        // id_componente = fk_componente JOIN [dbo].[operacoes] ON id_operacao =
                        // fk_operacao WHERE tipo = 'Processador' AND uso > 30.0 AND uso <= 50.0 ORDER
                        // BY id_registro DESC");
                        // List operacao = template.queryForList(
                        // "SELECT TOP (1) nome_operacao FROM [dbo].[log_registros] JOIN
                        // [dbo].[maquinas] ON id_maquina = fk_maquina JOIN [dbo].[componentes] ON
                        // id_componente = fk_componente JOIN [dbo].[operacoes] ON id_operacao =
                        // fk_operacao WHERE tipo = 'Processador' AND uso > 30.0 AND uso <= 50.0 ORDER
                        // BY id_registro DESC");
                        // List gerente = template.queryForList(
                        // "SELECT TOP (1) nome_gerente FROM [dbo].[log_registros] JOIN [dbo].[maquinas]
                        // ON id_maquina = fk_maquina JOIN [dbo].[componentes] ON id_componente =
                        // fk_componente JOIN [dbo].[operacoes] ON id_operacao = fk_operacao WHERE tipo
                        // = 'Processador' AND uso > 30.0 AND uso <= 50.0 ORDER BY id_registro DESC");

                        codigoUrgencia = "Atenção";
                        String descricao = "";
                        descricao = "O estado da CPU da máquina está em atenção.";
                        ...
                        // id_maquina = fk_maquina JOIN [dbo].[componentes] ON id_componente =
                        // fk_componente JOIN [dbo].[operacoes] ON id_operacao = fk_operacao WHERE tipo
                        // = 'Processador' AND uso > 50.0 AND uso <= 75.0 ORDER BY id_registro DESC");
                        // List localidade = template.queryForList(
                        // "SELECT TOP (1) localidade_maquina FROM [dbo].[log_registros] JOIN
                        // [dbo].[maquinas] ON id_maquina = fk_maquina JOIN [dbo].[componentes] ON
                        // id_componente = fk_componente JOIN [dbo].[operacoes] ON id_operacao =
                        // fk_operacao WHERE tipo = 'Processador' AND uso > 50.0 AND uso <= 75.0 ORDER
                        // BY id_registro DESC");
                        // List operacao = template.queryForList(
                        // "SELECT TOP (1) nome_operacao FROM [dbo].[log_registros] JOIN
                        // [dbo].[maquinas] ON id_maquina = fk_maquina JOIN [dbo].[componentes] ON
                        ...
                        String descricao = "";
                        descricao = "CPU em estado emergencial.";
                        ...
                        // id_maquina = fk_maquina JOIN [dbo].[componentes] ON id_componente =
                        // fk_componente JOIN [dbo].[operacoes] ON id_operacao = fk_operacao WHERE tipo
                        // = 'Processador' AND uso > 75.0 ORDER BY id_registro DESC");
                        // List localidade = template.queryForList(
                        // "SELECT TOP (1) localidade_maquina FROM [dbo].[log_registros] JOIN
                        // [dbo].[maquinas] ON id_maquina = fk_maquina JOIN [dbo].[componentes] ON
                        // id_componente = fk_componente JOIN [dbo].[operacoes] ON id_operacao =
                        // fk_operacao WHERE tipo = 'Processador' AND uso > 75.0 ORDER BY id_registro
                        // DESC");
                        // List operacao = template.queryForList(
                        // "SELECT TOP (1) nome_operacao FROM [dbo].[log_registros] JOIN
                        // [dbo].[maquinas] ON id_maquina = fk_maquina JOIN [dbo].[componentes] ON
                        ...
                        descricao = "O estado da CPU da máquina está em alerta.";
                ...
                if (percentualRAM >= 85.0) {
                        ...
                        // id_maquina = fk_maquina JOIN [dbo].[componentes] ON id_componente =
                        // fk_componente JOIN [dbo].[operacoes] ON id_operacao = fk_operacao WHERE tipo
                        // = 'Memoria Ram' AND uso > 85.0 ORDER BY id_registro DESC");
                        // List localidade = template.queryForList(
                        // "SELECT TOP (1) localidade_maquina FROM [dbo].[log_registros] JOIN
                        // [dbo].[maquinas] ON id_maquina = fk_maquina JOIN [dbo].[componentes] ON
                        // id_componente = fk_componente JOIN [dbo].[operacoes] ON id_operacao =
                        // fk_operacao WHERE tipo = 'Memoria Ram' AND uso > 85.0 ORDER BY id_registro
                        // DESC");
                        // List operacao = template.queryForList(
                        // "SELECT TOP (1) nome_operacao FROM [dbo].[log_registros] JOIN
                        // [dbo].[maquinas] ON id_maquina = fk_maquina JOIN [dbo].[componentes] ON
                        ...
                        codigoUrgencia = "Emergência";
                        String descricao = "O estado da Memoria RAM da máquina está muito acima do normal.";
